<x-app-layout>
    ...
    <div class="flex flex-row items-center justify-between gap-0.5">
        <h3 class="text-white uppercase">{{ __('teams.Manage teams') }}</h3>
        <a class="button w-fc text-white" href="{{ route('teams.create') }}">{{ __('teams.Create team') }}</a>
    </div>
            ...
            @foreach($teams as $team)
                <div class="flex flex-row items-center justify-between">
                    ...
                    <div class="flex flex-row items-center gap-4">
                        ...
                        @if($team->is_system === false)
                            <a class="flex flex-row items-center gap-1 button" href="{{ route('teams.edit', $team) }}">
                            ...
                            <form class="flex flex-row items-center gap-1 button-nocolor bg-red-900" method="POST" action="{{ route('teams.destroy', $team) }}">
                                @csrf
                                @method('DELETE')
                        ...
                        @else
                            <span class="badge whitespace-nowrap mb-2 inline-block text-sm bg-slate-800 hover:bg-slate-700 text-white border border-slate-700 py-1 px-3 rounded transition-all">
