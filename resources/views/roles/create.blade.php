<x-app-layout>
<form class="flex flex-col gap-4 item-bg text-white p-4" method="POST" action="{{ route('roles.store') }}">
    @csrf

    <div class="flex flex-col gap-1">
        <label for="role_name">{{ __('roles.Role name') }}</label>
        <input class="innerp" type="text" id="role_name" name="name" autocomplete="true" autofocus>
    </div>
    ...
    <div class="flex flex-row items-center flex-wrap gap-2">
        @foreach($teams as $team)
            <div class="relative flex flex-wrap items-center">
                ...
                <label class="cursor-pointer pl-2 text-slate-500 peer-disabled:cursor-not-allowed peer-disabled:text-slate-400" for="{{ $team->name }}"> {{ $team->name }} </label>
