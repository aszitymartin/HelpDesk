<x-app-layout>
    ...
    <form class="flex flex-col gap-4 item-bg text-white p-4" method="POST" action="{{ route('users.update', $user) }}">
        ...
        <div class="flex flex-col gap-1">
            ...
            <input class="innerp" type="text" id="full_name" name="name" placeholder="Name" value="{{ $user->name }}" required>
        </div>
        <div class="flex flex-col gap-1">
            <label for="email">Email</label>
            <input class="innerp" type="email" id="email" name="email" placeholder="Email" value="{{ $user->email }}" required>
        </div>

        <br />
        <h3 class="block font-medium text-white">New password</h3>
        <div class="flex flex-col gap-1">
            <label for="password">Password</label>
            <input class="innerp" type="password" id="password" name="password" placeholder="Password">
        </div>
        <div class="flex flex-col gap-1">
            <label for="password_confirmation">Confirm Password</label>
            <input class="innerp" type="password" id="password_confirmation" name="password_confirmation" placeholder="Password">
        </div>

        <br />
        
        <label for="select" class="block text-sm/6 font-medium text-white">{{ __('teams.teams') }}</label>
        <div x-data="teamRoleSelector({{ $user->team_id }}, @js($user->roles->pluck('name')))">
            ...
            <div class="flex flex-col gap-2">
                ...
                <template x-for="role in roles" :key="role.name">
                    <div class="relative flex items-center">
                        ...
                        <label class="pl-2 text-slate-300" x-text="role.name"></label>

                    </div>
                </template>

                <span x-show="roles.length === 0" class="text-slate-400">
                    No roles for this team
                </span>
    ...
    <script>
        function teamRoleSelector(initialTeamId, initialRoles) {
            return {
                ...
                init() {
                    this.loadRoles(this.teamId);
                },

                async onTeamChange(event) {
                    const newTeamId = event?.target?.value ?? event?.detail?.value ?? null;

                    if (!newTeamId) return;

                    this.teamId = newTeamId;

                    await this.loadRoles(newTeamId);

                    this.selectedRoles = [];
                },

                async loadRoles(teamId) {
