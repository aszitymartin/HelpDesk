...
<x-app-layout>
    ...
    <section class="flex flex-col gap-4">
        ...
        <div class="flex flex-row items-center justify-between gap-0.5">
            <h3 class="text-white uppercase">{{ __('permissions.manage permissions') }}</h3>
            <a class="button w-fc text-white" href="{{ route('permissions.create') }}">{{ __('permissions.create permission') }}</a>
        </div>
        ...
        @if(count($permissions) > 0)
            @foreach($permissions as $permission)
                <div class="flex flex-row items-center justify-between gap-0.5">
                    ...
                    <div class="flex flex-row items-center gap-4">
                        ...
                        <a class="flex flex-row items-center gap-1 button" href="{{ route('permissions.edit', $permission) }}">
                        ...
                        <form class="flex flex-row items-center gap-1 button-nocolor bg-red-900" method="POST" action="{{ route('permissions.destroy', $permission) }}">
                            @csrf
                            @method('DELETE')
            ...
            @endforeach
        @else
            <span>{{ __('permissions.No permissions') }}</span>
        @endif
        </div>
    </section>
